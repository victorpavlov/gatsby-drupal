...
class Profile extends React.Component {
  ...
  async componentDidMount() {
    ...
    try {
      let response = await fetch(`http://drupal-gatsby.docksal/oauth/debug`, options);
    ...
    if (userInfo.id) {
      try {
        ...
        let response = await fetch(`http://drupal-gatsby.docksal/jsonapi/user/user?filter[uid][value]=${userInfo.id}`, options);
    ...
    if (profile.drupal_internal__uid === +userInfo.id) {
